...
namespace DIY4UMazeGame.GameEngine.GameInfo.Items.ItemObject
{
    ...
    public class IronSword : Item, IWeapon, IStationary, IPrintable, ICarryable, IDropable, ITakable,
         IAvailable
    {
        static int instanceNo = 0;

        public int hitPower { get; set; } = 2;
        public int capableHit { get; set; } = 90;

        public int col { get; set; } = 0;
        public int row { get; set; } = 0;
        public char IconChar { get; set; } = 'G';
        public ConsoleColor ItemColor { get; set; } = ConfigManager.IronSwordColor;

        public IronSword(string itemname_in, string descriptions_in, int col_in, int row_in, string key)
        {
            ...
            itemtype = ITEM_TYPE.Iron_Sword;
            IconChar = ConfigManager.IronSwordIcon;

            capableHit = 380;
        }

        public IronSword(string descriptions_in, int col_in, int row_in, string key)
        {
            itemname = "Iron Sword";
            descriptions = descriptions_in;
            col = col_in;
