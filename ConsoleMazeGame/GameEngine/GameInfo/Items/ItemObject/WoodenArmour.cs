...
namespace DIY4UMazeGame.GameEngine.GameInfo.Items.ItemObject
{
    ...
    class WoodenArmour : Item, IShield, IStationary, IPrintable, ICarryable, IDropable, ITakable,
         IAvailable
    {
        static int instanceNo = 0;

        public int shieldPower { get; set; } = 100; //No of Hit this shield can take
        public int shieldLevel { get; set; } = 20; //The decrease in number of hit everytime it block a hit

        public int col { get; set; } = 0;
        public int row { get; set; } = 0;
        public char IconChar { get; set; } = 'G';
        public ConsoleColor ItemColor { get; set; } = ConfigManager.WoodenArmourColor;


        public WoodenArmour(string itemname_in, string descriptions_in, int col_in, int row_in, string key)
        {
            ...
            itemtype = ITEM_TYPE.Wooden_Armour;
            IconChar = ConfigManager.WoddenArmourIcon;

            shieldLevel = 1;
        }

        public WoodenArmour(string descriptions_in, int col_in, int row_in, string key)
        {
            itemname = "Wooden Armour";
            descriptions = descriptions_in;
            col = col_in;
            row = row_in;
            itemname = itemname + "-" + instanceNo;
